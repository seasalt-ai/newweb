 ...
 */
export function getLocalizedOrganizationDescription(
  language: string,
  fallback: string
): string {
  const descriptions: Record<string, string> = {
    ...
    // Chinese (Traditional)
    "zh-TW":
      "領先的 AI 對話智能平台，為各種規模的企業提供全通路客戶溝通解決方案。",

    // Chinese (Simplified)
    "zh-CN":
      "领先的 AI 对话智能平台，为各种规模的企业提供全渠道客户沟通解决方案。",
...
export function createOrganizationSchema(
  ...
  return {
    ...
    contactPoint: {
      ...
      availableLanguage:
        availableLanguages || orgInfo.contactPoint.availableLanguage,
    },
    ...
    availableLanguage: (availableLanguages || orgInfo.availableLanguage).map(
      (lang) => ({
        "@type": "Language",
        name: lang,
...
export function createSoftwareApplicationSchema({
  ...
  const productUrl = createLocalizedUrl(
    baseUrl,
    language,
    path || `/${productKey}`
  );
  ...
  const offer = createOfferSchema({
    ...
    priceCurrency:
      customOffer?.priceCurrency || productInfo.offers.priceCurrency,
    availability:
      customOffer?.availability ||
      (productInfo.offers.availability as SchemaAvailability),
  ...
  return {
    ...
    screenshot: productInfo.screenshots?.map(
...
export function createBreadcrumbListSchema(
  ...
  return {
    ...
    itemListElement: breadcrumbs.map((crumb, index) => ({
      ...
      name: crumb.name,
      item:
        crumb.url || (crumb.path ? `${baseUrl}${crumb.path}` : `${baseUrl}`),
    })),
  };
}
