...
在此次更新之前，企業必須做出選擇：要么留在 Business App（但其在自動化和多代理支援方面存在限制），要么遷移到 API（失去應用程式存取權、聊天記錄，有時甚至失去企業電話號碼）。共存消除了這種權衡，使組織能夠在不中斷現有客戶關係或工作流程的情況下擴展其訊息業務。
...
| WhatsApp Cloud API    | 啟用自動化、多代理存取、CRM 整合和進階分析                                                   |
...
| 僅 API                | WhatsApp Cloud API 無應用程式存取權；支援完整自動化、多代理、CRM 整合                         | 大型企業、高流量營運                       |
...
- **共用收件匣：** 多代理儀表板允許團隊分配、追蹤 WhatsApp 對話並進行協作
- **AI/聊天機器人整合：** 自動流程、潛在客戶資格認定和客戶支援機器人與人工代理並行運作
...
- **統一的客戶體驗：** 無論客戶是與人工代理還是機器人聊天，都能透過單一 WhatsApp 號碼進行互動
- **多代理協作：** 多個團隊成員可以透過 CRM 管理對話，而主管或專家可以根據需要透過應用程式加入
- **自動化路由：** 聊天機器人對查詢進行分類，並將其路由到合適的代理或部門
- **無縫交接：** 代理可以接手由自動化啟動的對話，並保留完整的上下文和聊天記錄
...
- 透過應用程式將複雜案件轉交給人工代理
- 實現 24 小時客戶支援，減少代理閒置時間，並獲得 9.1/10 的客戶滿意度評分
...
- **自動分級：** 機器人處理常見問題和例行查詢，並在需要時轉接給代理
...
代理和銷售人員可以從單一儀表板管理所有 WhatsApp 對話，並在應用程式和 CRM 之間即時同步。這實現了：

- **更快的回應時間：** 多個代理可以協作處理高優先順序的對話
- **無縫交接：** 對話可以在自動化和人工代理之間轉移，而不會丟失上下文
- **分析和報告：** 主管追蹤回應時間、解決率和客戶滿意度

### 9.2 混合式互動

- **手動 + 自動化：** 代理透過應用程式處理複雜或高價值的對話；常規任務透過 API 自動化處理  
- **個人化：** 人類代理可在需要時添加個人化元素，而機器人則處理重複性查詢  
- **連續性：** 無論管道或代理為何，客戶都能體驗到統一的品牌聲音  
...
- **解決方案：** WhatsApp 共存搭配聊天機器人自動化和人類代理備援  
- **成果：** 24 小時支援、減少代理閒置時間、9.1/10 的客戶滿意度、零放棄率  

### 11.2 零售業：D2C 美妝品牌  

- **挑戰：** 由於單一代理的 WhatsApp 設置，導致回應時間緩慢且錯失銷售機會  
- **解決方案：** 透過共存實現多代理協作，搭配 CRM 整合和自動化潛在客戶分配  
