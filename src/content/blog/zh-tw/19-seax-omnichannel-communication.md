...
首先，“全通路”到底是什麼意思？分解來看，“omni”是一個前綴，意思是“所有”，而“管道”是您可以與客戶互動的各種平台。因此，簡單地說，“全通路通訊”意味著能夠透過任何和所有可用管道進行通訊。不僅如此，全通路通訊還意味著管道之間的體驗是無縫的。在Agent商方面，來自所有管道的通訊都呈現在一個統一的介面中。對於客戶來說，他們的互動數據在各個管道之間是持久的。

傳統的呼叫中心通常只支援電話呼叫。透過多個管道（例如電子郵件、網路聊天和電話）與客戶聯繫的更高級的聯絡中心擁有多管道聯絡中心。然而，僅僅因為聯絡中心利用多個管道並不意味著他們的體驗是無縫的。在多管道聯絡中心，不同的管道可能透過單獨的平台存取，和/或客戶數據可能無法跨管道連結。相比之下，全通路聯絡中心讓Agent商可以隨時隨地追蹤客戶對話，而不會被鎖定在一個管道或分散在數十個平台上。
...
如果您想直接觀看演示，請觀看我們演示 SeaX 全通路通訊的短影片。在本部落格的其餘部分，我們將逐步介紹訊息和呼叫如何從各種管道路由到 SeaX 中的Agent商。我們還將分享開箱即用的支援管道，並討論如何擴展 SeaX 以覆蓋新管道。
...
SeaX 建立在 [Twilio Flex](https://www.twilio.com/flex) 之上，這是一個基於雲端的聯絡中心，利用 Twilio 的雲端通訊平台。Twilio 為 SeaX 提供了基本的構建塊，例如電信基礎設施、訊息和任務路由以及基本的聯絡中心 UI。現在讓我們追蹤傳入使用者訊息的生命週期，看看 SeaX 如何使用基本的 Twilio 架構結合自訂組件將訊息路由到 SeaX 平台上的即時Agent商。
...
<center>
<img src="/images/blog/19-seax-omnichannel-communication/3-studio-flow.png" alt="一個簡單的 Studio Flow，可將訊息路由到聊天機器人或即時Agent商。"/>

*一個簡單的 Studio Flow，可將訊息路由到聊天機器人或即時Agent商。*
</center>

訊息被 Twilio 接收後，需要將其路由到正確的位置。為此，我們使用 [Twilio Studio Flows](https://www.twilio.com/studio) 來確定是提供自動回覆、將訊息轉發到聊天機器人、將使用者與即時Agent商聯繫還是執行其他操作。

在上面提供的簡單範例中，所有傳入的訊息都將轉發到聊天機器人，除非它們包含「即時Agent商」一詞，在這種情況下，使用者將被轉移到 SeaX 平台上的即時Agent商。
...
訊息傳輸到 SeaX 後，下一步是決定哪個Agent商將接收它。[Twilio 的 TaskRouter](https://www.twilio.com/taskrouter) 將訊息和電話呼叫等任務分配給 SeaX 中最能處理它們的Agent商。可以為 SeaX 中的每個Agent商分配技能，例如他們會說哪種語言、他們在哪個部門工作、他們是否應該處理 VIP 客戶等。TaskRouter 將檢查有關使用者和訊息的已知信息，然後選擇最合適的員工來處理問題。上一步中的 Studio Flow 可以自訂以獲取其他信息（例如首選語言），並且客戶信息可以在對話和管道之間保留，以確保他們的體驗是無縫的。
...
最後，傳入的訊息將顯示給 SeaX 平台上相應的Agent商。Agent商可以同時處理來自多個管道的多個任務。在上圖中，一個Agent商有來電、Facebook 訊息和網路聊天訊息。Agent商可以接受任務或拒絕任務，以將其傳遞給下一個可用的Agent商。

# 支援的管道

希望現在更清楚什麼是全通路通訊，以及它如何增強使用者和Agent商的體驗。最後一個問題是：開箱即用支援哪些管道？
...
感謝您花時間閱讀 SeaX 雲端聯絡中心如何利用全通路通訊提供無縫的客戶和Agent商體驗。請繼續關注我們的下一篇部落格文章，該文章將探討成為「分散式聯絡中心」意味著什麼。如果您有興趣立即了解更多信息，請填寫我們的 [預約演示表單](https://meetings.hubspot.com/seasalt-ai/seasalt-meeting) 以親身體驗 SeaX 平台。
