...
- [SeaX 知识库](#seax-knowledge-base)
    - [即时Agent商的嵌入式用户界面](#embedded-user-interface-for-live-agents)
...
通常，知识库通过内容管理系统或知识管理系统实施和维护。这些系统可以根据组织的需求而规模各异，从简单的文档管理器到功能丰富的服务，包括发布工作流、受众定位、协作工具等。虽然这些系统在不同方面具有多功能性，但它们几乎总是旨在通过与网页或应用程序的互动来访问。对于客户服务Agent的特定用例（他们通常将知识库作为协助客户的主要资源之一），需要与联络中心软件紧密整合，以使Agent能够尽可能无缝地处理用户查询。
...
自然地，由于我们的知识库引擎是专门为联络中心应用程序设计的，因此它与 SeaX 平台原生整合，以便Agent商可以无缝访问知识库，同时处理呼叫和消息。无需切换窗口，无需翻阅标签，无需导航嵌套网页。
...
但到目前为止我们所展示的仍然是手动搜索。即时Agent商忙于与客户互动，每次需要信息时手动输入搜索到知识库中会浪费宝贵的时间。因此，SeaX 知识库带来的最大附加价值是针对文本和语音互动的即时自动搜索。
...
每次有新的用户消息传入时，知识库都会使用客户的精确消息自动查询。即时地，当客户说话时，Agent商将获得最新的知识库文章建议供其参考。
...
<center>
<img src="/images/blog/22-seax-knowledge-base/kb-response-template.png" alt="Agent商使用 SeaX 知识库生成的回复模板一键回复客户。"/>

*Agent商使用 SeaX 知识库生成的回复模板一键回复客户。*
</center>

知识库的搜索结果还有一个额外功能，有助于加快Agent商对基于文本的互动的响应速度。当Agent商找到相关的知识库文章时，他们只需点击标题左侧的“+”图标即可将回复模板插入到其聊天窗口中。在后端，每次搜索知识库时，它都会根据建议的知识库文章中最相关的信息生成对用户问题的书面回复，并包含任何支持链接。这可以大大提高Agent商的响应时间，因为Agent商不再从空白开始。相反，他们已经将知识库文章中的重要信息显示在其聊天窗口中，因此他们只需编辑并发送即可。
...
如需一对一演示，或了解 Seasalt.ai 如何根据您的业务需求定制解决方案，请填写我们的 [预约演示表单](https://calendar.app.google/GcdRQv1DVoiDaoCT6)。