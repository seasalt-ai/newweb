...
const FeatureComparison = () => {
  ...
  const features = useMemo(() => [
    {
      ...
      otherTools: t('seachat.featureComparison.pricing.chatbotWebsite'),
      seachat: 'TRUE'
    },
    {
      feature: t('seachat.featureComparison.features.chatWidget', 'Chat widget with live agent'),
      replaces: ['tawk.to', 'freshchat'],
      otherTools: t('seachat.featureComparison.pricing.chatWidget'),
      seachat: 'TRUE'
    },
    {
      ...
      otherTools: t('seachat.featureComparison.pricing.whatsappMessenger'),
      seachat: 'TRUE'
    },
    {
      feature: t('seachat.featureComparison.features.multiCalendar', 'Multi Calendar bookings'),
      replaces: ['Calendly', 'Cal.com'],
      otherTools: t('seachat.featureComparison.pricing.multiCalendar'),
      seachat: 'TRUE'
    },
    {
      feature: t('seachat.featureComparison.features.phoneCallAI', 'Inbound phone call AI agents'),
      replaces: ['Retell', 'Bland AI', 'Synthflow'],
      otherTools: t('seachat.featureComparison.pricing.phoneCallAI'),
      seachat: 'TRUE'
    },
    {
      feature: t('seachat.featureComparison.features.smsActions', 'Built-in actions - SMS'),
      replaces: ['textline', 'textrequest', 'salesmsg'],
      otherTools: t('seachat.featureComparison.pricing.smsActions'),
      seachat: 'TRUE'
    },
    {
      feature: t('seachat.featureComparison.features.emailActions', 'Built-in actions - Email'),
      replaces: ['Sendgrid', 'MailChimp'],
      otherTools: t('seachat.featureComparison.pricing.emailActions'),
      seachat: 'TRUE'
    },
    {
      feature: t('seachat.featureComparison.features.phoneNumber', 'Purchase phone number'),
      replaces: ['Twilio'],
      otherTools: t('seachat.featureComparison.pricing.phoneNumber'),
      seachat: 'TRUE'
    },
    {
      feature: t('seachat.featureComparison.features.agencyMode', 'Agency Mode'),
      replaces: t('seachat.featureComparison.replaces.uniqueToSeachat'),
      otherTools: '',
      seachat: t('seachat.featureComparison.seachatValues.agencyMode')
    },
    {
      feature: t('seachat.featureComparison.features.bulkSMS', 'Bulk outbound SMS, Call, WhatsApp'),
      replaces: [],
      otherTools: '',
      seachat: t('seachat.featureComparison.seachatValues.bulkSMS')
    },
    {
      feature: t('seachat.featureComparison.features.unlimitedContacts', 'Unlimited Contacts'),
      replaces: ['ManyChat'],
      otherTools: t('seachat.featureComparison.pricing.unlimitedContacts'),
      seachat: t('seachat.featureComparison.seachatValues.unlimitedContacts')
    },
    {
      feature: t('seachat.featureComparison.features.knowledgeBase', 'Extra knowledge base'),
      replaces: ['Synthflow'],
      otherTools: t('seachat.featureComparison.pricing.knowledgeBase'),
      seachat: t('seachat.featureComparison.seachatValues.knowledgeBase')
    },
    {
      feature: t('seachat.featureComparison.features.extraChatbot', 'Extra chatbot'),
      replaces: ['chatbase'],
      otherTools: t('seachat.featureComparison.pricing.extraChatbot'),
      seachat: t('seachat.featureComparison.seachatValues.extraChatbot')
    }
  ], [t]);
  ...
  const renderCompanyLogos = (companies: string[] | string) => {
    if (typeof companies === 'string') {
      if (companies === t('seachat.featureComparison.replaces.uniqueToSeachat')) {
        return (
          <div className="flex items-center justify-center">
            <Star className="w-4 h-4 text-yellow-500 mr-2" />
            <span className="text-xs text-yellow-700 font-medium">{t('seachat.featureComparison.uniqueToSeachat')}</span>
          </div>
        );
      }
              ...
              <thead className="bg-gray-100">
                <tr>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900 border-r border-gray-200">
                    {t('seachat.featureComparison.table.headers.features')}
                  </th>
                  <th className="px-6 py-4 text-center text-sm font-semibold text-gray-900 border-r border-gray-200">
                    {t('seachat.featureComparison.table.headers.replaces')}
                  </th>
                  <th className="px-6 py-4 text-center text-sm font-semibold text-gray-900 border-r border-gray-200">
                    {t('seachat.featureComparison.table.headers.otherTools')}
                  </th>
                  <th className="px-6 py-4 text-center text-sm font-semibold text-teal-900 bg-teal-50">
                    {t('seachat.featureComparison.table.headers.seachat')}
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                ...
                <tr className="bg-gradient-to-r from-blue-50 to-teal-50 text-xs">
                  <td className="px-6 py-3 text-center font-medium text-blue-700 border-r border-gray-200">
                    {t('seachat.featureComparison.table.explanations.valueYouGet')}
                  </td>
                  <td className="px-6 py-3 text-center font-medium text-blue-700 border-r border-gray-200">
                    
                  </td>
                  <td className="px-6 py-3 text-center font-medium text-red-700 border-r border-gray-200">
                    {t('seachat.featureComparison.table.explanations.priceYouWouldPay')}
                  </td>
                  <td className="px-6 py-3 text-center font-medium text-teal-700 bg-teal-100">
                    {t('seachat.featureComparison.table.explanations.priceYouPay')}
                  </td>
                </tr>
              </tbody>
            </table>
              ...
              <div key={index} className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center py-2 border-b border-gray-100">
                    <span className="text-sm text-gray-600">{t('seachat.featureComparison.mobile.replaces')}</span>
                  ...
                  <div className="flex justify-between items-center py-2 border-b border-gray-100">
                    <span className="text-sm text-gray-600">{t('seachat.featureComparison.mobile.otherTools')}</span>
                  ...
                  <div className="flex justify-between items-center py-2 bg-teal-50 rounded-lg px-3">
                    <span className="text-sm font-medium text-teal-700">{t('seachat.featureComparison.mobile.seachat')}</span>
            ...
            <div className="bg-gradient-to-r from-gray-100 to-teal-100 rounded-xl p-6 font-bold">
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center">
                  <div className="text-sm text-gray-600 mb-2">{t('seachat.featureComparison.mobile.otherToolsTotal')}</div>
                  <div className="text-lg font-bold text-red-600">{t('seachat.featureComparison.mobile.totalPrice')}</div>
                  <div className="text-xs text-red-600 mt-1">{t('seachat.featureComparison.mobile.priceYouWouldPay')}</div>
                </div>
                <div className="text-center">
                  <div className="text-sm text-teal-700 mb-2">{t('seachat.featureComparison.mobile.seachatTotal')}</div>
                  <div className="text-2xl font-bold text-teal-700">{t('seachat.featureComparison.mobile.seachatTotalPrice')}</div>
                  <div className="text-xs text-teal-700 mt-1">{t('seachat.featureComparison.mobile.priceYouPay')}</div>
                </div>
              </div>
            </div>
          </div>
