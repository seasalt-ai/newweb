      ...
      <div className="max-w-7xl mx-auto flex items-center justify-center">
        <div className="flex items-center space-x-2 text-center">
          ...
          <span className="font-semibold text-lg">
            {t('common.phoneBanner.callText', 'Call')} <a 
              href="tel:+1-762-244-3688" 
              className="text-yellow-300 hover:text-yellow-200 underline decoration-2 underline-offset-2"
            >
              +1 (SMB)-AI-AGENT
            </a> {t('common.phoneBanner.bookMeeting', 'to book a meeting with the SeaVoice AI agent.')}
          </span>
          <div className="flex items-center space-x-1 text-yellow-300">
            <Clock className="w-4 h-4" />
            <span className="text-sm font-medium">{t('common.phoneBanner.available247', 'Available 24/7')}</span>
          </div>
        </div>
      </div>
    </div>
