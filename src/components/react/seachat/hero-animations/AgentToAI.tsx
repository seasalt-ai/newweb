...
const AgentToAI: React.FC<AgentToAIProps> = ({ lang, translations }) => {
  ...
  const humanMessages = [
    getText('seachat.heroAnimations.agentToAI.humanMessages.0', 'How can I help you today?'),
    getText('seachat.heroAnimations.agentToAI.humanMessages.1', 'Let me check that for you'),
    getText('seachat.heroAnimations.agentToAI.humanMessages.2', 'I understand your concern'),
    getText('seachat.heroAnimations.agentToAI.humanMessages.3', 'Here\'s what I recommend'),
  ];

  const aiMessages = [
    getText('seachat.heroAnimations.agentToAI.aiMessages.0', 'I can help you with that instantly'),
    getText('seachat.heroAnimations.agentToAI.aiMessages.1', 'Based on your history, I suggest...'),
    getText('seachat.heroAnimations.agentToAI.aiMessages.2', 'I\'ve analyzed your request'),
    getText('seachat.heroAnimations.agentToAI.aiMessages.3', 'The optimal solution is...'),
  ];
  ...
  }, [phase, aiConfidence, nextId, humanMessages, aiMessages]);
  ...
  return (
    <div className="relative w-full h-96 bg-gradient-to-br from-purple-50 to-blue-50 rounded-2xl overflow-hidden p-6">
              ...
              <div className={`flex-1 p-3 rounded-lg shadow-sm ${
                ...
                {conv.learned && (
                  <div className="flex items-center mt-2 space-x-1">
                    ...
                    <span className="text-xs text-green-600 font-medium">{getText('seachat.heroAnimations.agentToAI.learned', 'Learned')}</span>
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="absolute bottom-4 left-4 right-4 text-center">
        <h3 className="text-lg font-bold text-gray-800">{getText('seachat.heroAnimations.agentToAI.title', 'Human to AI Evolution')}</h3>
        <p className="text-sm text-gray-600">{getText('seachat.heroAnimations.agentToAI.subtitle', 'AI learns from human interactions')}</p>
      </div>
    </div>
  );
};
