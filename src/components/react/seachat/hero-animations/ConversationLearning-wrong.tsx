    ...
    <div className="relative w-full h-96 bg-gradient-to-br from-purple-50 to-indigo-100 rounded-2xl overflow-hidden p-6">
      ...
      <div className="relative h-32 mb-6 bg-white/70 backdrop-blur-sm rounded-xl border border-white/50 overflow-hidden">
              ...
              if (distance < 40) {
                return (
                  <line
                    ...
                    strokeWidth="1"
                    className={`transition-opacity duration-500 ${node1.pulse || node2.pulse ? 'animate-pulse' : ''}`}
                  />
                );
              }
              return null;
        ...
        {knowledgeNodes.map((node) => (
          <div
            key={`label-${node.id}`}
            className={`absolute text-xs font-medium pointer-events-none transition-all duration-300 ${
              node.pulse ? 'text-purple-700 font-bold' : 'text-gray-600'
            }`}
            style={{
              left: `${node.x}%`,
              top: `${node.y + 12}%`,
      ...
      {/* Active Insight */}
      <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-white/50 mb-4 shadow-lg">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white/70 backdrop-blur-sm rounded-lg p-3 text-center shadow-sm transition-all duration-300 hover:shadow-md">
