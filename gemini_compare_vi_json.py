...
# 1. Key order
if list(en_data.keys()) != list(vi_data.keys()):
...
missing, extra = compare_keys(en_data, vi_data)
if missing:
    print(f"Verification Failed: Missing keys in vi.json: {missing}")
if extra:
    print(f"Verification Failed: Extra keys in vi.json: {extra}")
if not missing and not extra:
    print("Verification Successful: No missing or duplicated keys found.")

# 3. Line count
en_lines = len(en_json_str.strip().split('\n'))
vi_lines = len(vi_json_str.strip().split('\n'))
if en_lines != vi_lines:
    print(f"Verification Failed: Line count mismatch. en.json has {en_lines} lines, vi.json has {vi_lines} lines.")
else:
    print("Verification Successful: Line counts match.")

# 4. Formatting
if en_json_str.startswith('{\n  "') and vi_json_str.startswith('{\n  "'):
