...
# 1. Key order
if list(en_data.keys()) != list(pl_data.keys()):
...
missing, extra = compare_keys(en_data, pl_data)
if missing:
    print(f"Verification Failed: Missing keys in pl.json: {missing}")
if extra:
    print(f"Verification Failed: Extra keys in pl.json: {extra}")
if not missing and not extra:
    print("Verification Successful: No missing or duplicated keys found.")

# 3. Line count
en_lines = len(en_json_str.strip().split('\n'))
pl_lines = len(pl_json_str.strip().split('\n'))
if en_lines != pl_lines:
    print(f"Verification Failed: Line count mismatch. en.json has {en_lines} lines, pl.json has {pl_lines} lines.")
else:
    print("Verification Successful: Line counts match.")

# 4. Formatting
if en_json_str.startswith('{\n  "') and pl_json_str.startswith('{\n  "'):
