...
# 1. Key order
if list(en_data.keys()) != list(pt_data.keys()):
...
missing, extra = compare_keys(en_data, pt_data)
if missing:
    print(f"Verification Failed: Missing keys in pt.json: {missing}")
if extra:
    print(f"Verification Failed: Extra keys in pt.json: {extra}")
if not missing and not extra:
    print("Verification Successful: No missing or duplicated keys found.")

# 3. Line count
en_lines = len(en_json_str.strip().split('\n'))
pt_lines = len(pt_json_str.strip().split('\n'))
if en_lines != pt_lines:
    print(f"Verification Failed: Line count mismatch. en.json has {en_lines} lines, pt.json has {pt_lines} lines.")
else:
    print("Verification Successful: Line counts match.")

# 4. Formatting
if en_json_str.startswith('{\n  "') and pt_json_str.startswith('{\n  "'):
