...
# 1. Key order
if list(en_data.keys()) != list(ja_data.keys()):
...
missing, extra = compare_keys(en_data, ja_data)
if missing:
    print(f"Verification Failed: Missing keys in ja.json: {missing}")
if extra:
    print(f"Verification Failed: Extra keys in ja.json: {extra}")
if not missing and not extra:
    print("Verification Successful: No missing or duplicated keys found.")

# 3. Line count
en_lines = len(en_json_str.strip().split('\n'))
ja_lines = len(ja_json_str.strip().split('\n'))
if en_lines != ja_lines:
    print(f"Verification Failed: Line count mismatch. en.json has {en_lines} lines, ja.json has {ja_lines} lines.")
else:
    print("Verification Successful: Line counts match.")

# 4. Formatting
if en_json_str.startswith('{\n  "') and ja_json_str.startswith('{\n  "'):
