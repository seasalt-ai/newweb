...
# 1. Key order
if list(en_data.keys()) != list(hi_data.keys()):
...
missing, extra = compare_keys(en_data, hi_data)
if missing:
    print(f"Verification Failed: Missing keys in hi.json: {missing}")
if extra:
    print(f"Verification Failed: Extra keys in hi.json: {extra}")
if not missing and not extra:
    print("Verification Successful: No missing or duplicated keys found.")

# 3. Line count
en_lines = len(en_json_str.strip().split('\n'))
hi_lines = len(hi_json_str.strip().split('\n'))
if en_lines != hi_lines:
    print(f"Verification Failed: Line count mismatch. en.json has {en_lines} lines, hi.json has {hi_lines} lines.")
else:
    print("Verification Successful: Line counts match.")

# 4. Formatting
if en_json_str.startswith('{\n  "') and hi_json_str.startswith('{\n  "'):
