...
# -*- coding: utf-8 -*-
"""
批量修复多语言博客中的"Untitled"问题
检测和修复以下问题：
1. 缺少title字段的文件
2. title字段为空或"Untitled"的文件
3. 内容语言与目录语言不匹配的文件
4. frontmatter格式错误的文件
"""
...
def check_file(file_path, expected_lang):
    """检查单个文件的问题"""
    issues = []
    ...
    # 检查内容语言
    if body:
        detected_lang = detect_language_from_text(body[:1000])  # 检查前1000个字符
...
def scan_all_files():
    ...
    for lang in SUPPORTED_LANGUAGES:
        ...
        for file_path in lang_files:
            total_files += 1
            issues = check_file(file_path, lang)
...
def fix_common_issues():
    ...
    for lang in SUPPORTED_LANGUAGES:
        ...
        for file_path in lang_path.glob('*.md'):
            ...
            # 修复title为"Untitled"的情况
            title = frontmatter.get('title', '')
            if title.lower() == 'untitled':
                # 从文件名生成title
                filename = file_path.stem
                new_title = filename.replace('-', ' ').replace('_', ' ').title()
                ...
                print(f"  ✅ 修复 {file_path.name}: 生成新title '{new_title}'")

if __name__ == "__main__":
    print("🚀 多语言博客问题检测和修复工具")
    print("=" * 60)
