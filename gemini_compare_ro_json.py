...
# 1. Key order
if list(en_data.keys()) != list(ro_data.keys()):
...
missing, extra = compare_keys(en_data, ro_data)
if missing:
    print(f"Verification Failed: Missing keys in ro.json: {missing}")
if extra:
    print(f"Verification Failed: Extra keys in ro.json: {extra}")
if not missing and not extra:
    print("Verification Successful: No missing or duplicated keys found.")

# 3. Line count
en_lines = len(en_json_str.strip().split('\n'))
ro_lines = len(ro_json_str.strip().split('\n'))
if en_lines != ro_lines:
    print(f"Verification Failed: Line count mismatch. en.json has {en_lines} lines, ro.json has {ro_lines} lines.")
else:
    print("Verification Successful: Line counts match.")

# 4. Formatting
if en_json_str.startswith('{\n  "') and ro_json_str.startswith('{\n  "'):
