...
# 1. Key order
if list(en_data.keys()) != list(fil_data.keys()):
...
missing, extra = compare_keys(en_data, fil_data)
if missing:
    print(f"Verification Failed: Missing keys in fil.json: {missing}")
if extra:
    print(f"Verification Failed: Extra keys in fil.json: {extra}")
if not missing and not extra:
    print("Verification Successful: No missing or duplicated keys found.")

# 3. Line count
en_lines = len(en_json_str.strip().split('\n'))
fil_lines = len(fil_json_str.strip().split('\n'))
if en_lines != fil_lines:
    print(f"Verification Failed: Line count mismatch. en.json has {en_lines} lines, fil.json has {fil_lines} lines.")
else:
    print("Verification Successful: Line counts match.")

# 4. Formatting
if en_json_str.startswith('{\n  "') and fil_json_str.startswith('{\n  "'):
