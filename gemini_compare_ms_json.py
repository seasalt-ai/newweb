...
# 1. Key order
if list(en_data.keys()) != list(ms_data.keys()):
...
missing, extra = compare_keys(en_data, ms_data)
if missing:
    print(f"Verification Failed: Missing keys in ms.json: {missing}")
if extra:
    print(f"Verification Failed: Extra keys in ms.json: {extra}")
if not missing and not extra:
    print("Verification Successful: No missing or duplicated keys found.")

# 3. Line count
en_lines = len(en_json_str.strip().split('\n'))
ms_lines = len(ms_json_str.strip().split('\n'))
if en_lines != ms_lines:
    print(f"Verification Failed: Line count mismatch. en.json has {en_lines} lines, ms.json has {ms_lines} lines.")
else:
    print("Verification Successful: Line counts match.")

# 4. Formatting
if en_json_str.startswith('{\n  "') and ms_json_str.startswith('{\n  "'):
