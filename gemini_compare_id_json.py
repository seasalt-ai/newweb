...
# 1. Key order
if list(en_data.keys()) != list(id_data.keys()):
...
missing, extra = compare_keys(en_data, id_data)
if missing:
    print(f"Verification Failed: Missing keys in id.json: {missing}")
if extra:
    print(f"Verification Failed: Extra keys in id.json: {extra}")
if not missing and not extra:
    print("Verification Successful: No missing or duplicated keys found.")

# 3. Line count
en_lines = len(en_json_str.strip().split('\n'))
id_lines = len(id_json_str.strip().split('\n'))
if en_lines != id_lines:
    print(f"Verification Failed: Line count mismatch. en.json has {en_lines} lines, id.json has {id_lines} lines.")
else:
    print("Verification Successful: Line counts match.")

# 4. Formatting
if en_json_str.startswith('{\n  "') and id_json_str.startswith('{\n  "'):
