...
/**
 * Translation Keys Cleanup Script
 * 
 * This script analyzes the current usage of translation keys in the codebase
 * and removes any unused keys from the en.json translation file.
 * 
 * Features:
 * - Scans all React components for t() function calls and Trans component usage
 * - Identifies all translation keys currently defined in en.json
 * - Creates a backup before making changes
 * - Removes unused keys from en.json
 * - Generates a report of what was cleaned up
 */
...
const CONFIG = {
  ...
  outputDir: 'translation-cleanup',
  extensions: ['.tsx', '.ts', '.jsx', '.js'],
  // Skip certain directories from scanning
...
class TranslationCleaner {
  ...
  createCleanedTranslationFile() {
    if (this.unusedKeys.size === 0) {
      console.log(`${colors.green}🎉 No unused keys found! Translation file is already clean.${colors.reset}`);
      return;
    }
  ...
  generateMarkdownReport(report) {
    return `# Translation Keys Cleanup Report
...
✅ **Successfully cleaned up ${report.summary.unused_keys_removed} unused translation keys!**

### Removed Keys

The following unused keys were removed from en.json:

\`\`\`
${report.unused_keys_removed.join('\n')}
\`\`\`
` : '✅ **No cleanup needed!** All translation keys are currently in use.'}

## Backup Information

A backup of the original translation file has been created:
...
If you need to restore the original translation file:
...
---
*Report generated by Translation Keys Cleanup Script*
`;
  }

  // Main execution function
  async run() {
    console.log(`${colors.bright}${colors.blue}🧹 Translation Keys Cleanup Script${colors.reset}\n`);
    ...
    try {
      ...
      if (this.unusedKeys.size > 0) {
        ...
        console.log(`${colors.green}🎉${colors.reset} No unused keys found - translation file is clean!`);
      }
